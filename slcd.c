...
void slcdInitialize(void){
	...
	PORTB->PCR[21] = PORT_PCR_MUX(1UL);
	PORTB->PCR[22] = PORT_PCR_MUX(1UL);
	PORTB->PCR[23] = PORT_PCR_MUX(1UL);
	PORTC->PCR[17] = PORT_PCR_MUX(1UL);
	PORTC->PCR[18] = PORT_PCR_MUX(1UL);
	PORTD->PCR[0] = PORT_PCR_MUX(1UL);
...
/*----------------------------------------------------------------------------
  Set Error table for errors 
 1 == in OCT out of range 
 2 == in BIN out of range
 *----------------------------------------------------------------------------*/
void setErrorTable(uint16_t t[4],uint8_t x) // x is a error number
{
	t[3]=0x0E;
	t[2]=16;
	t[1]=16;
...
void slcdDisplay(uint16_t value ,uint16_t format){	
	...
	if ((format==8)&&(value>4095))
	{
			setErrorTable(tab,1);
	}else if((format==2)&&(value>15))
	{
...
/*----------------------------------------------------------------------------
  Funtion for displaying dots where
  1 to 3 means after which number dot will be display
	4 means col display
 *----------------------------------------------------------------------------*/
void slcdSetDot(uint8_t which_dot)
{
	if(which_dot>0){
		LCD->WF8B[LCD_Front_Pin[((2*which_dot)-1)]] |= (LCD_S_DEC);
	} 
}
/*----------------------------------------------------------------------------
  Clear sLCD
...
void slcd2display(uint8_t player,uint8_t score) 
{
	if((player!=1)&&(player!=2))
	{
		...
		if(score==60)
		{
			score=0xA0;
			switch(player){
				case 1: slcdSet(score%16,LCD_SECOND_DIGIT);
								score/=16;
								slcdSet(score%16,LCD_FIRST_DIGIT);
				break;
				case 2: slcdSet(score%16,LCD_FOURTH_DIGIT); 
								score/=16;
								slcdSet(score%16,LCD_THIRD_DIGIT);
				break;
			}
		}
		else{
			...
			switch(player){
				case 1: slcdSet(score%10,LCD_SECOND_DIGIT);
								score/=10;
								slcdSet(score%10,LCD_FIRST_DIGIT);
				break;
				case 2: slcdSet(score%10,LCD_FOURTH_DIGIT); 
								score/=10;
								slcdSet(score%10,LCD_THIRD_DIGIT);
				break;
				}
		}
	}
